...
use log4tc_core::{AppSettings, LoggingConfig, LogFormat, ReceiverConfig, ServiceConfig, OutputConfig};
use serde_json::json;
...
fn test_config_complete_structure() {
    let config = AppSettings {
        logging: LoggingConfig {
            ...
            output_path: Some(PathBuf::from("/var/log/log4tc.log")),
        },
...
fn test_logging_path_with_env_vars() {
    ...
    let config = LoggingConfig {
        log_level: "info".to_string(),
        format: LogFormat::Text,
        output_path: Some(PathBuf::from("/var/log/log4tc/app.log")),
    };

    assert!(config.output_path.is_some());
    let path = config.output_path.unwrap();
    assert!(path.to_string_lossy().contains("log4tc"));
}
