...
mod tests {
    ...
    fn test_service_config_defaults() {
        ...
        assert_eq!(config.name, "Log4TcService");
    ...
    fn test_logging_config_text_format() {
        let config = LoggingConfig {
            ...
            output_path: Some(std::path::PathBuf::from("/var/log/log4tc.log")),
        };

        assert_eq!(config.log_level, "info");
        assert_eq!(config.format, LogFormat::Text);
        assert!(config.output_path.is_some());
        assert_eq!(config.output_path.unwrap().to_string_lossy(), "/var/log/log4tc.log");
    }
