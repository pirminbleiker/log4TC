# log4tc-benches - Performance Benchmarking Suite

High-performance benchmarking suite for log4tc-rust using criterion.rs.

## Overview

This crate provides comprehensive benchmarks for the log4tc-rust logging system, measuring:
...
```bash
cargo bench -p log4tc-benches --bench otel_conversion
cargo bench -p log4tc-benches --bench ads_parser
cargo bench -p log4tc-benches --bench log_entry_creation
cargo bench -p log4tc-benches --bench end_to_end
```

### Run Specific Test and Show Details
```bash
cargo bench -p log4tc-benches --bench otel_conversion -- convert_typical --verbose
```
...
```rust
use log4tc_benches::LogEntryFixtures;
...
```rust
use log4tc_benches::AdsFixtures;
...
cargo bench -p log4tc-benches --bench otel_conversion -- --save-baseline v0

# Later: compare against baseline
cargo bench -p log4tc-benches --bench otel_conversion -- --baseline v0
```
...
```bash
# Before optimization
cargo bench -p log4tc-benches > before.txt

# After optimization
cargo bench -p log4tc-benches > after.txt
...
cargo bench -p log4tc-benches -- --sample-size 500

# Decrease for faster (less accurate) results
cargo bench -p log4tc-benches -- --sample-size 50
```
...
cargo bench -p log4tc-benches -- "parse"
...
cargo bench -p log4tc-benches -- --warm-up-time 5

# Measure time per iteration
cargo bench -p log4tc-benches -- --measurement-time 10
```
...
```yaml
- name: Run benchmarks
  run: cargo bench -p log4tc-benches --bench otel_conversion

- name: Compare to baseline
  run: |
    cargo bench -p log4tc-benches --bench otel_conversion -- --baseline main || true
...
```bash
# Run with backtrace
RUST_BACKTRACE=1 cargo bench -p log4tc-benches --bench ads_parser
```
...
1. **Establish baseline**
   ```bash
   cargo bench -p log4tc-benches -- --save-baseline v0
   ```
