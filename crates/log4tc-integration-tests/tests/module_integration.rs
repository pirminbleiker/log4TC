...
use log4tc_core::{AppSettings, LogLevel};
use log4tc_otel::OtelMapping;
use serde_json::json;
...
fn test_parser_core_otel_pipeline() {
    ...
    // Step 2: Convert to LogEntry (from log4tc-core)
    let mut log_entry = log4tc_core::LogEntry::new(
    ...
    // Step 3: Convert to OTEL LogRecord (from log4tc-otel)
...
fn test_loglevel_across_modules() {
    // Use LogLevel from log4tc-core in multiple contexts
    let levels = vec![
        LogLevel::Trace,
        LogLevel::Debug,
        LogLevel::Info,
    ...
    for level in levels {
        // Create entry with log4tc-core
        let entry = log4tc_core::LogEntry::new(
            "src".to_string(),
            "host".to_string(),
            format!("Test {:?}", level),
            "test".to_string(),
...
fn test_serde_integration() {
    ...
    let mut entry = log4tc_core::LogEntry::new(
    ...
    // Deserialize back
    let deserialized: log4tc_core::LogEntry = serde_json::from_str(&json).expect("Failed to deserialize");
...
fn test_type_conversions() {
    ...
    // Convert to OTEL with log4tc-otel
    let record = log4tc_core::LogRecord::from_log_entry(entry);
